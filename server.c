...
void *handle_connection(void *arg) {
  ...
  //     bzero(request_room.password, BUFFER_SIZE);
  //
  //     if (!pb_decode_delimited(&input, RequestJoinRoom_fields,
  //     &request_room)) {
  ...
  //
  //         } else {
  //           send_msg_to_other_client(temp->head,
  ...
  //       temp = temp->next;
  //     }
  //   } else if (request_type.type == RequestType_EXIT_ROOM) {
...
int main() {
  ...
  int check = pthread_create(&thread_id, &attr, handle_connection, &arg);
